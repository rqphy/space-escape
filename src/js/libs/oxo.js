window.oxo = {
	...
	animation: {
		...
		setPosition(element, position) {
			var transform = element.style.transform.replace(
				/translate\(.+\)/,
				""
			)
							...
							interval = setInterval(function () {
								window.requestAnimationFrame(function () {
									if (pressed.length) {
										oxo.animation.move(
											element,
											oxo.inputs.getDirectionFromPressedKeys(
												pressed
											),
											pixels,
											false
	...
	elements: {
		...
		onCollisionWithElementOnce(element, target, action) {
			return oxo.elements.onCollisionWithElement(
				element,
				target,
				action,
				true
	...
	inputs: {
		...
		listenAllKeys() {
			document.addEventListener("keydown", function (event) {
				let listener = oxo.inputs.keysListeners[event.keyCode]
				if (listener) {
					listener.action()
		...
		getDirectionFromPressedKeys(pressed) {
			...
			;["left-up", "left-down", "right-up", "right-down"].forEach(
				function (dir) {
					if (
	...
	screens: {
		...
		loadScreen(name, action) {
			return fetch("../../screens/" + name + ".html").then(function (
				response
			) {
				if (response.ok) {
	...
	log(message) {
		console.log(
			"%c OXO: " + message,
			"background-color: gold; padding: 5px"
		)
	},
